        ...
        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="single-blog-post">
                                    ...
                                    <div class="blog-post-content">
                                        <h3><a href="{{route('frontend.blog_detail', $blog->id)}}">{{$blog->{'title_' . $language} }}</a></h3> {{--
                                        <p>{!! $blog->{'description_' .$language} !!} </p>--}}
                                        <a href="{{route('frontend.blog_detail', $blog->id)}}" class="read-more">
                                            <span class="left"><i class="icofont-rounded-double-right"></i></span> @lang('SuperAdmin/backend.read_more')
            ...
            <div class="col-md-4">
                <div class="recent-side">
                    <h6>{{__('Frontend.search')}}</h6>

                    <input onkeyup="getBlogSearch($(this).val())" class="form-control" type="search" placeholder="Search" aria-label="Search">

                    <div class="recent mt-3" id="hideblog">
...
<script>
    function getBlogSearch(value) {
        if(value != '') {
            $.get('blog/search/' + value, {}, function(data) {
                $("#hideblog").html(data.result);
            });
        } else {
            $.get('blog/search/' + value, {}, function(data) {
                $("#hideblog").html(data.default);
            });
        }
    }
</script>
