        ...
        <div class="card">
            <div class="card-body table table-responsive">
                <div style="text-align: center;">
                    <h1 class="card-title">{{__('SuperAdmin/backend.course_program_details')}}</h1>
                </div>
                ...
                <table class="table table-hover table-bordered">
                    <thead>
                        <tr>
                            <th># </th>
                            <th> {{__('SuperAdmin/backend.program_registration_free')}} </th>
                            <th> {{__('SuperAdmin/backend.program_duration')}} </th>
                            <th> {{__('SuperAdmin/backend.age_range')}} </th>
                            <th> {{__('SuperAdmin/backend.courier_fee')}} </th>
                            <th> {{__('SuperAdmin/backend.about_courier')}} </th>
                            <th> {{__('SuperAdmin/backend.program_cost')}} </th>

                            <th> {{__('SuperAdmin/backend.program_duration_start')}} </th>
                            <th> {{__('SuperAdmin/backend.program_duration_end')}} </th>
                            <th> {{__('SuperAdmin/backend.program_start_date')}} </th>
                            <th> {{__('SuperAdmin/backend.program_end_date')}} </th>
                            ...
                            <th> {{__("SuperAdmin/backend.x_week_selected")}} </th>
                            <th> {{__('SuperAdmin/backend.week_start_date')}} </th>
                            <th> {{__('SuperAdmin/backend.week_end_date')}} </th>
                            <th> {{__('SuperAdmin/backend.week_free')}} </th>
                            ...
                            <th> {{__('SuperAdmin/backend.program_under_age_fee')}} </th>
                    ...
                    <tbody>
                        @foreach($course_programs as  $course_program)
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn-primary btn-sm" type="button" href="{{route("superadmin.course_program_underage_details", $course_program->unique_id)}}">{{__("SuperAdmin/backend.click_here")}}</a>
                                </td>
                                <td>
                                    <div class="btn-group">
                                        <a href="{{route('superadmin.course_program_delete', $course_program->unique_id)}}" type="button" class="btn btn-danger btn-sm fa fa-trash">
                                        </a>
                                    </div>
                                </td>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="exampleModalLabel5">{{__('SuperAdmin/backend.course_program_update')}}</h5>
                                        ...
                                        <form method="post" action="{{route('superadmin.course_program_update')}}" id="submit_form">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="form-group">
                                                    <label>{{__('SuperAdmin/backend.age_range')}}</label>
                                                ...
                                                <div class="form-group">
                                                    <label>{{__('SuperAdmin/backend.about_courier')}} {{__('SuperAdmin/backend.in_english')}}</label>
                                                    <textarea id="about_couriers_textarea" class="form-control">{{ $course_program->about_courier }}</textarea>
                                                </div>

                                                <div class="form-group">
                                                    <label>{{__('SuperAdmin/backend.about_courier')}} {{__('SuperAdmin/backend.in_arabic')}}</label>
                                                ...
                                                <div class="form-group">
                                                    <label>{{__('SuperAdmin/backend.program_start_week')}}</label>
                                                    <input type="date" name="program_start_date" value="{{ $course_program->program_start_date}}" class="form-control">
                                                </div>

                                                <div class="form-group">
                                                    <label>{{__('SuperAdmin/backend.program_end_week')}}</label>
                                                ...
                                                <div class="form-group">
                                                    <label>{{__('SuperAdmin/backend.how_many_week_free')}}</label>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button id="magic" onclick="getContent('inondhu', 'nandhu'); submit_courseprogram($(this))" type="button" class="btn btn-primary">{{__('SuperAdmin/backend.update_changes')}}</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
