        ...
        <div class="card">
            <div class="card-body">
                <form id="affiliateForm" class="forms-sample" method="POST" action="{{ route('superadmin.user.affiliate.update', $affiliate->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label>{{__('Admin/backend.address')}}*:</label>
                            <input class="form-control" value="{{ $affiliate->address }}" type="text" name="address" placeholder="{{__('Admin/backend.address')}}">
                        </div>
                        <div class="form-group col-md-6">
                            <label>{{__('Admin/backend.post_code')}}*:</label>
                            <input class="form-control" value="{{ $affiliate->post_code }}" type="text" name="post_code" placeholder="{{__('Admin/backend.post_code')}}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label>{{__('Admin/backend.country')}}*:</label>
                            <select onchange="changeCountry()" class="form-control" name="country[]" id="country_name">
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" {{ in_array($country->id, $affiliate->country ?? []) ? 'selected' : ''}}>{{ app()->getLocale() == 'en' ? $country->name : $country->name_ar }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            <label>{{__('Admin/backend.city')}}*:</label>
                            <select class="form-control" name="city[]" id="city_name">
                                @foreach ($countries as $country)
                                    @if (($affiliate->country && in_array($country->id, $affiliate->country)) || (!$affiliate->country && $loop->iteration == 1))
                                        @foreach ($country->cities as $city)
                                            <option value="{{ $city->id }}" {{ in_array($city->id, $affiliate->city ?? []) ? 'selected' : ''}}>{{ app()->getLocale() == 'en' ? $city->name : $city->name_ar }}</option>
                                        @endforeach
                                    @endif
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label>{{__('Admin/backend.commission_rate')}}*:</label>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            <label>{{__('Admin/backend.account_number')}}*:</label>
