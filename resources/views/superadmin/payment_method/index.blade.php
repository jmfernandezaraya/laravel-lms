                    ...
                    <tbody>
                        @foreach($payment_methods as $payment_method)
                            <tr>
                                ...
                                <td>{{ get_language() == 'en' ? $payment_method_list[$payment_method->key]['en'] : $payment_method_list[$payment_method->key]['ar'] }}</td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            @if (can_manage_payment_method() || can_edit_payment_method())
                                                <a href="{{route('superadmin.payment_method.edit', $payment_method->unique_id)}}" class="btn btn-info btn-sm fa fa-pencil"></a>
                                            @endif
                                            
                                            @if (can_manage_payment_method() || can_delete_payment_method())
                                                @if (can_manage_payment_method())
                                                    @if(!$payment_method->is_default)
                                                        <form action="{{route('superadmin.payment_method.set_default', $payment_method->unique_id)}}" method="POST">
                                                            @csrf
                                                ...
                                                <form action="{{route('superadmin.payment_method.destroy', $payment_method->unique_id)}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
