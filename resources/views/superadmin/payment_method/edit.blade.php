...
@section('title')
    {{__('Admin/backend.update_payment_method')}}
@endsection
...
@section('content')
        ...
        <div class="card">
            <div class="card-body">
                <div style="text-align: center;">
                    <h1 class="card-title">{{__('Admin/backend.update_payment_method')}}</h1>
                    <change>
                        <div class="english">
    ...
    <div class="page-content">
        <div class="card">
            <div class="card-body">
                <form id="paymentMethodForm" class="forms-sample" method="post" action="{{route('superadmin.payment_method.update', $payment_method->unique_id)}}">
                    {{csrf_field()}}
                    @method('PUT')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="key" class="form-control">
                                @foreach ($payment_method_list as $method_key => $method_value)
                                    <option value="{{ $method_key }}" {{ $payment_method->key == $method_key ? 'selected' : '' }} data-logo="{{ $method_value['logo'] }}">{{ app()->getLocale() == 'en' ? $method_value['en'] : $method_value['ar'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                    ...
                    <div class="row">
                        <div class="form-group col-md-6 store-id">
                            ...
                            <input class="form-control" value="{{ $payment_method->store_id }}" type="text" name="store_id" placeholder="{{__('Admin/backend.store_id')}}">
                        </div>
                        <div class="form-group col-md-6 store-auth-key">
                            <label for="store_auth_key">{{__('Admin/backend.store_auth_key')}}</label>
                            <input class="form-control" value="{{ $payment_method->store_auth_key }}" type="text" name="store_auth_key" placeholder="{{__('Admin/backend.store_auth_key')}}">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="form-group col-md-6 store-secret-key">
                            ...
                            <input class="form-control" value="{{ $payment_method->store_secret_key }}" type="text" name="store_secret_key" placeholder="{{__('Admin/backend.store_secret_key')}}">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="mode">{{__('Admin/backend.mode')}}</label>
                            <select name="test_mode" class="form-control">
                                <option value="0" {{ $payment_method->test_mode ? '' : 'selected' }}>{{__('Admin/backend.real_mode')}}</option>
                                <option value="1" {{ $payment_method->test_mode ? 'selected' : '' }}>{{__('Admin/backend.test_mode')}}</option>
                            </select>
                        </div>
                    </div>
