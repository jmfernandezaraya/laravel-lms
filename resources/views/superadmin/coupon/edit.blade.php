...
@section('content')
    ...
    <div class="page-content">
        <div class="card">
            <div class="card-body">
                <form id="formaction" class="forms-sample" method="POST" action="{{ route('superadmin.coupon.update', $coupon->unique_id) }}">
                    {{csrf_field()}}
                    @method('PUT')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" name="affiliate_id">
                                ...
                                @foreach($affiliates as $affiliate)
                                    <option value="{{ $affiliate->id }}" {{ $affiliate->id == $coupon->affiliate_id ? 'selected' : ''}}>{{ $affiliate->{'first_name_' . get_language()} }}&nbsp;{{ $affiliate->{'last_name_' . get_language()} }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                                        ...
                                        <tbody>
                                            @foreach($courses as $course)
                                                <tr>
                                                    ...
                                                    <td><input type="checkbox" data-id="{{$course->unique_id}}" {{ in_array($course->unique_id, $coupon->course_unique_ids) ? 'checked' : '' }} /></td>
