        ...
        <div class="card">
            <div class="card-body table table-responsive">
                <div style="text-align: center;">
                    <h1 class="card-title">{{__('Admin/backend.medical_details')}}</h1>
                </div>
                ...
                <table class="table table-hover table-bordered">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th> {{__('Admin/backend.medical_fees_per_week')}} </th>
                            <th> {{__('Admin/backend.medical_start_week')}} </th>
                            <th> {{__('Admin/backend.medical_end_week')}} </th>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="exampleModalLabel5">{{__('Admin/backend.update_medical_fees')}}</h5>
                                        ...
                                        <form method="post" action="{{ auth('superadmin')->check() ? route('admisuperadminn.medical_update') : route('schooladmin.medical_update') }}">
                                            ...
                                            <div class="modal-body">
                                                <div class="form-group">
                                                    <label>{{__('Admin/backend.medical_insurance_note')}} {{__('Admin/backend.in_english')}}</label>
                                                ...
                                                <div class="form-group">
                                                    <label>{{__('Admin/backend.medical_insurance_note') {{__('Admin/backend.in_arabic')}}</label>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button onclick  ="getContent('textarea_medical_insurance_note_en', 'input_medical_insurance_note_en'); getContent('textarea_medical_insurance_note_ar', 'input_medical_insurance_note_ar')" type="submit" class="btn btn-primary">{{__('Admin/backend.update_changes')}}</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
