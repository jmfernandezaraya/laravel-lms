        ...
        <div class="card">
            <div class="card-body">
                <div style="text-align: center;">
                    <h1 class="card-title">{{__('Admin/backend.edit_rating')}}</h1>
                    <change>
                        <div class="english">
            ...
            <div class="card-body">
                <form id ="formaction" class="forms-sample" method="post" action="{{ auth('superadmin')->check() ? route('superadmin.blog.update', $blog->id) : route('schooladmin.blog.update', $blog->id) }}">
                    ...
                    <div id="form2" class="forms-sample">
                        ...
                        <a class="btn btn-light" href="{{url()->previous()}}">{{__('Admin/backend.cancel')}}</a>
                        <button type="button" onclick="submitCommonForBlogForm($(this).parents().find('#formaction').attr('action'))" class="btn btn-primary">@lang('Admin/backend.submit')</button>
                    </div>
                </form>
            </div>
        </div>
