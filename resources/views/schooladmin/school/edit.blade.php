...
@section('js')
    @php
        $schoolupdateurl  = route('superadmin.school.update', $school_ar->unique_id);
    @endphp
    <script>
        ...
        <div class="card-body">
            ...
            <form id="form1" class="forms-sample" enctype="multipart/form-data" action="{{route('superadmin.schools.store')}}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input value="{{$school_en->email}}" name="email" type="text" class="form-control" id="exampleInputEmail3" placeholder="{{__('SuperAdmin/backend.school_email_address')}}">
                </div>
                @if($errors->has('email'))
            ...
            <form id="form2" class="forms-sample" enctype="multipart/form-data" method="post" action = "{{route('schooladmin.schools.store')}}">
                {{csrf_field()}}
                <div class="form-group">
                    <label for="exampleInputName1">{{__('SuperAdmin/backend.school_name')}}</label>
