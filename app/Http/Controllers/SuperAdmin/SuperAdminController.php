        ...
        \DB::transaction(function () use ($request, $requested_save, $image_name) {
            ...
            if (can_manage_user() || can_permission_user()) {
                $user->permission()->create([
                    ...
                    'payment_method_manager' => $request->payment_method_permission == 'manager',
                    'payment_method_add' => ($request->payment_method_permission == 'subscriber' && $request->payment_method_add) ?? 0,
                    'payment_method_edit' => ($request->payment_method_permission == 'subscriber' && $request->payment_method_edit) ?? 0,
                    'payment_method_delete' => ($request->payment_method_permission == 'subscriber' && $request->payment_method_delete) ?? 0,
